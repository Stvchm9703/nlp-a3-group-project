...
def download_dataset(dataset_dir):
    """Downloads the conllpp dataset from the HuggingFace
...
def extract_embeddings(config, vocab):
    ...
    embeddings_path = config["glove_file"]
    with open(embeddings_path, "rb") as _file:
        for line in _file:
    ...
    with open(save_path_map, "w", encoding="utf8") as _file:
        json.dump(word_to_idx, _file)
...
def download_dataset_dl(dataset_name):
    dt = load_dataset(
        dataset_name,
        cache_dir = f"{PROJECT_DIRTECTORY}/datasets/{dataset_name}"
    )
    return dt
